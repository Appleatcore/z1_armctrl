...
# from iros_ros_foundationpose.srv import Reset, getgoalandangle, planandgrippercontrol

import rospy
from geometry_msgs.msg import PoseStamped, Pose
from std_srvs.srv import Trigger
from arm_controller_srvs.srv import getgoalandangle, planandgrippercontrol, PlanToDefault, BackToHome
...
def execute_point_by_name(point_name, points_dict, service_name):
    ...
    if point_name not in points_dict:
        print(f"✗ 警告: 点位 '{point_name}' 未在 getgoalandangle 的响应中找到。跳过此点。")
...
def main():
    ...
    # 这些值将在标定步骤中被覆盖（如果成功）
    # !! 您必须在此处填入您想要的默认值 !!
    pos_x, pos_y, pos_z = 0.85, 0.0, 0.0
    input_roll  = 0.7  # 示例值，请修改
    input_pitch = 0.1   # 示例值，请修改
    ...
    if not (success and goal_response.call_success):
        print(f"✗ 'getgoalandangle' 服务调用失败 (Success: {success}, Response Success: {goal_response.call_success if goal_response else 'N/A'})")
        return 1
    
    print(f"✓ 'getgoalandangle' 服务成功, 收到 {len(goal_response.target_poses)} 个点位")
